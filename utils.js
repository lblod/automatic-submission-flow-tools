/**
 * @module utils
 * @description This module provides some utility functions used throughout the rest of the tools library or externally.
 */
...
// This code has been transcribed from the following to fit our needs: https://github.com/rubensworks/sparqljson-parse.js/blob/master/lib/SparqlJsonParser.ts
 ...
 * @public
 * @function
 * @param {object} jsonTerm - The contents of a binding from SPARQL JSON results. This is usually of the structure: `{ type: 'bnode|literal|uri', value: 'some value' }` but also deals with Virtuoso's 'typed-literal' key.
 ...
 * @public
 * @function
 * @param {object} rawBinding - This is an object from the array of bindings from the SPARQL JSON results. E.g. this could be of the structure `{ book: term, isbn: term, pages: term }` where every `term` is an object as described in {@link parseSparqlJsonTerm}.
 * @results {object} This is an object with the same keys as the input object, but where every value is parsed to the appropriate RDFJS term. E.g. as a continuation of the previous example: `{ book: namedNode, isbn: literal, pages: literal }`.
 */
...
/**
 * Parse a full SPARQL JSON results binding to RDFJS quad specification. This only works when the variables in the SPARQL query are 'subject', 'predicate' and 'object'. Think of this function as a shorthand for using {@link parseSparqlJsonBinding} and an extra map over the bindings to translate them into triples when dealing with results from CONSTRUCT queries or mu-authorization/delta-notifier results.
 *
 * @public
 * @function
 * @param {object} rawBinding - This is an object from the array of bindings from the SPARQL JSON results. E.g. this could be of the structure `{ s: term, p: term, o: term }` where every `term` is an object as described in {@link parseSparqlJsonTerm}.
 * @results {quad} This is an RDFJS quad representing the triple for the given binding.
 */
